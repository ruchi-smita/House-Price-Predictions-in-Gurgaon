...
for train_index, test_index in split.split(housing, housing['income_cat']):
    strat_train_set = housing.loc[train_index].drop("income_cat", axis=1)#we will work on the training set
    strat_test_set = housing.loc[test_index].drop("income_cat", axis=1)#set aside the test set

#we will work on the training set
...
#4. Sepatarating numerical and categorical columns
num_attribs = housing.drop("ocean_proximity", axis=1).columns.tolist()
cat_attribs = ["ocean_proximity"]

#5. let's create pipelines 
...
# print(f"The root mean squared error for Linear Regessioin is{lin_rmse}" )
...
# print(f"The root mean squared error for random_forest Regessioin is{random_forest_rmse}" )
